...
### openGL program
- flat shading : 폴리곤의 면 전체를 하나의 색상으로 결정하는 쉐이딩 방식.
- 기존에 쉐이더에서 쉐이딩 계산을 진행했던 것과 달리 C++ program 상에서 색상 계산 진행.
- 세 버텍스의 노말, 위치의 중점값을 기준으로 쉐이딩 계산을 진행하여 얻어낸 하나의 색상을 각 버텍스에 할당.
- 색상값을 쉐이더로 전달

### Shader
- 전달받은 버텍스와 그 버텍스의 색상을 레스터라이저로 넘겨서, 한 폴리곤 면이 모두 동일한 색상으로 커버됨. 

## 결과
![Alt text](./readmeData/Screen-Recording-2024-12-13-at-2.11.37-PM.gif)